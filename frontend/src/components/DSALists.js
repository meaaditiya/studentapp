...
const DSALists = () => {
  ...
  const fetchLists = async () => {
    try {
      const response = await fetch('http://192.168.1.35:5000/api/lists');
  ...
  const handleAddQuestion = async (listKey) => {
    try {
      const response = await fetch(`http://192.168.1.35:5000/api/lists/${listKey}/add`, {
        method: 'POST',
        headers: {
  ...
  const handleMoveQuestion = async (fromList, questionId, toList) => {
    try {
      const response = await fetch(
        `http://192.168.1.35:5000/api/lists/${fromList}/move/${questionId}/${toList}`,
        { method: 'PUT' }
      );
  ...
  const handleDeleteQuestion = async (listKey, questionId) => {
    ...
    try {
      const response = await fetch(`http://192.168.1.35:5000/api/lists/${listKey}/delete/${questionId}`, {
        method: 'DELETE',
      });
