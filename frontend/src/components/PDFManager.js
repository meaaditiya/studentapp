...
const PDFManager = () => {
  ...
  const fetchPDFs = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://personalstudentdiary.onrender.com/api/pdfs');
  ...
  const uploadPDF = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch('https://personalstudentdiary.onrender.com/api/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const deletePDF = async (id) => {
    if (window.confirm('Are you sure you want to delete this PDF?')) {
      try {
        setLoading(true);
        const response = await fetch(`https://personalstudentdiary.onrender.com/api/pdfs/${id}`, {
          method: 'DELETE',
        });
        ...
        {isViewerOpen && selectedPDF && (
          <div className="pdf-manager-viewer">
            ...
            <div className="pdf-manager-document-container">
              <Document
                file={`https://personalstudentdiary.onrender.com/api/pdfs/${selectedPDF._id}`}
