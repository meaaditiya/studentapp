...
const ExcelTables = () => {
  ...
  useEffect(() => {
    const fetchExcelTables = async () => {
      try {
        const response = await axios.get("https://studentapp-backend-ccks.onrender.com/tables");
  ...
  const saveExcelTable = async (headers, data, title) => {
    try {
      await axios.post("https://studentapp-backend-ccks.onrender.com/tables", { headers, data, title });
  ...
  const deleteExcelTable = async (title) => {
    ...
    try {
      ...
      const response = await axios.delete(`https://studentapp-backend-ccks.onrender.com/tables/${encodedTitle}`);
