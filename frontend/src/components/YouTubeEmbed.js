...
const YouTubeEmbed = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      const response = await axios.get('http://192.168.1.35:5000/api/videos');
  ...
  const addVideoLink = async () => {
    ...
    if (videoId && newTitle) {
      await axios.post('http://192.168.1.35:5000/api/videos', { videoId, title: newTitle });
  ...
  const deleteVideo = async (videoId) => {
    await axios.delete(`http://192.168.1.35:5000/api/videos/${videoId}`);
