...
function DailySchedule() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get("http://192.168.1.35:5000/tasks"); // Change URL to your IP address
    ...
    try {
      if (newTask.trim()) {
        if (editIndex !== null) {
          ...
          await axios.put(`http://192.168.1.35:5000/tasks/${tasks[editIndex]._id}`, updatedTask); // Change URL to your IP address
          ...
          // Add new task
          const response = await axios.post("http://192.168.1.35:5000/tasks", {
            text: newTask,
            completed: false,
            lastUpdated: new Date().toLocaleString(),
          });
  ...
  const toggleTask = async (index) => {
    try {
      ...
      await axios.put(`http://192.168.1.35:5000/tasks/${tasks[index]._id}`, {
        ...updatedTasks[index],
  ...
  const deleteTask = async (index) => {
    try {
      await axios.delete(`http://192.168.1.35:5000/tasks/${tasks[index]._id}`); // Change URL to your IP address
