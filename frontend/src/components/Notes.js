...
function Notes() {
  ...
  const fetchNotes = async () => {
    try {
      const response = await axios.get("https://personalstudentdiary.onrender.com/api/notes"); // Updated to your IP address
  ...
  const addNote = async () => {
    if (noteText.trim()) {
      ...
      try {
        const response = await axios.post("https://personalstudentdiary.onrender.com/api/notes", {
          text: noteText,
          date,
          dayTime,
        });
  ...
  const deleteNote = async (id) => {
    try {
      await axios.delete(`https://personalstudentdiary.onrender.com/api/notes/${id}`); // Updated to your IP address
  ...
  const editNote = async (index) => {
    ...
    if (newNoteText !== null && newNoteText.trim()) {
      ...
      try {
        const response = await axios.put(`https://personalstudentdiary.onrender.com/api/notes/${notes[index]._id}`, {
          text: newNoteText,
          date,
          dayTime,
        });
