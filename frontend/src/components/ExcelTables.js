...
const ExcelTables = () => {
  ...
  useEffect(() => {
    const fetchExcelTables = async () => {
      try {
        const response = await axios.get("http://192.168.1.42:5000/tables");
  ...
  const saveExcelTable = async (headers, data, title) => {
    try {
      await axios.post("http://192.168.1.42:5000/tables", { headers, data, title });
  ...
  const deleteExcelTable = async (title) => {
    try {
      ...
      const response = await axios.delete(`http://192.168.1.42:5000/tables/${encodedTitle}`);
