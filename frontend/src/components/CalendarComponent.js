...
const CalendarTest = () => {
  ...
  useEffect(() => {
    axios.get(`http://192.168.1.41/api/calendar-tasks/${formattedDate}`)
  ...
  const handleAddTask = () => {
    ...
    axios.post('http://192.168.1.41/api/calendar-tasks', { date: formattedDate, task: newTask })
      .then(response => {
  ...
  const handleDeleteTask = (taskToDelete) => {
    ...
    axios.delete(`http://192.168.1.41/api/calendar-tasks`, {
      params: {
        date: formattedDate,
        task: taskToDelete
      }
