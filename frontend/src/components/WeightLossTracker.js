...
const WeightLossTracker = () => {
  ...
  const handleHeightSubmit = async (e) => {
    ...
    try {
      await Promise.all(updatedEntries.map(entry =>
        fetch(`https://personalstudentdiary.onrender.com/api/weight-entries/${entry._id}`, {
          method: 'PUT',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://personalstudentdiary.onrender.com/api/weight-entries', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteEntry = async (id) => {
    try {
      const response = await fetch(`https://personalstudentdiary.onrender.com/api/weight-entries/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchEntries();
  ...
  const fetchTargetWeight = async () => {
    try {
      const response = await fetch('https://personalstudentdiary.onrender.com/api/target-weight');
      const data = await response.json();
      if (data && data.weight) {
