...
const ExcelTables = () => {
  ...
  useEffect(() => {
    const fetchExcelTables = async () => {
      try {
        const response = await axios.get("https://personalstudentdiary.onrender.com/tables");
  ...
  const saveExcelTable = async (headers, data, title) => {
    try {
      await axios.post("https://personalstudentdiary.onrender.com/tables", { headers, data, title });
  ...
  const deleteExcelTable = async (title) => {
    ...
    try {
      ...
      const response = await axios.delete(`https://personalstudentdiary.onrender.com/tables/${encodedTitle}`);
